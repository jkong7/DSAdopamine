    ...
    if file_path: #If user selected a file
        try:
            ...
            if 't' in data.columns and 'y' in data.columns:  # Check if file contains 't' and 'y' columns
                ...
                # Call this function immediately after setting the default parameter values
                update_function_form_and_parameter_boundaries()
...
def reset_parameters():
    ...
    parameter_values.update(default_values)
    # Print the parameter values for debugging
    print("Parameter values after reset:", parameter_values)
    # Update the UI to reflect the reset values
    update_function_form_and_parameter_boundaries()

# Reset button
reset_button = ttk.Button(file_frame, text="Reset Parameters", command=reset_parameters)  # Button to reset parameters
reset_button.pack(side=tk.RIGHT, padx=5, pady=5)  # Pack button on the right with padding
...
def update_stored_parameters(param, var, min_entry, max_entry):
    ...
    # Update the display in the entry widgets
        min_entry.delete(0, tk.END)
        min_entry.insert(0, str(parameter_values[param + "_min"]))
        max_entry.delete(0, tk.END)
        max_entry.insert(0, str(parameter_values[param + "_max"]))
...
def update_results_display(results=None, *args):
    selected_function = function_var.get()  # Get selected function
    print(f"update_results_display called with results: {results}")

    # Ensure results is a dictionary
    if results is None or not isinstance(results, dict):
        results = {}
        print("Results set to an empty dictionary")

    # Clear the results frame every time the user selects a function
    for child in results_frame.winfo_children():  # Clear previous results display
        child.destroy()
    print("Results frame cleared")
    ...
    for i, result in enumerate(result_labels):  # Initialize the results variables and create labels and entries for them
        result_value = results.get(result, "")  # Get result value if results are provided
        results_vars[result] = tk.StringVar(value=result_value)  # Initialize with result value or empty string
        print(f"Setting {result} to {result_value}")
...
# Example fitting results dictionary for testing
fitting_results = {
    "B": 0.5,
    "A1": 1.2,
    "tau1": 0.8,
...
# Add a print statement before calling the function to ensure this code block is executed
print("Calling update_results_display with sample data...")
# Update the results display with the fitting results for testing
update_results_display(results=fitting_results)
print("Called update_results_display with sample data.")
